...
export default {
  ...
  container: ({ props, context, state }) => ({
    class: [
      ...
      {
        'hover:border-yellow-500': !props.invalid,
        'ring ring-yellow-400/50': state.focused
      },
      // Transition
    ...
    return {
      class: [
        ...
        'border',
        {
          ...
          ' border-slate-300': !props.multiple && !props.invalid,
          'border-0 bg-transparent': props.multiple
        },
        // Invalid State
        { 'border-red-500': props.invalid },
        // States
        { 'focus:outline-none focus:outline-offset-0 focus:ring focus:ring-yellow-400/50': !props.multiple },
  ...
  token: {
    class: ['inline-flex items-center', 'py-1.5 px-3', 'rounded-[1.14rem]', 'bg-slate-200', 'text-slate-700', 'cursor-default']
  },
  removeTokenIcon: {
  ...
  dropdownButton: {
    root: {
      class: ['relative', 'items-center inline-flex text-center align-bottom', 'rounded-r-md', 'px-4 py-3 leading-[normal]', 'text-yellow-inverse', 'bg-yellow', 'border border-yellow', 'focus:outline-none focus:outline-offset-0 focus:ring', 'hover:bg-yellow-500 hover:border-yellow-500', 'focus:ring-yellow-400/50']
    }
  },
  loadingIcon: {
    class: ['text-slate-500', 'absolute top-[50%] right-[0.5rem] -mt-2 animate-spin']
  },
  panel: {
    class: ['bg-white', 'text-slate-700', 'border-0', 'rounded-md', 'shadow-md', 'max-h-[200px] overflow-auto']
  },
  list: {
  ...
  itemGroup: {
    class: ['font-bold', 'm-0 p-3', 'bg-white', 'text-slate-800', 'cursor-auto']
  },
  emptyMessage: {
    class: ['leading-[normal]', 'py-3 px-5', 'text-slate-800', 'bg-transparent']
  },
  transition: {
