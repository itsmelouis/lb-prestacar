...
const handleSubmit = async () => {
  ...
  if (!firstName.value || !name.value || !email.value || !message.value) {
  ...
  if (message.value.length < 50) {
    errorMessage.value = 'Le message doit dépasser 50 caractères.';
    loading.value = false;
    return;
  }

  if (!isCondition.value) {
    errorMessage.value = 'Vous devez accepter les conditions d\'utilisation générales de Prestacar.';
    loading.value = false;
    return;
  }
